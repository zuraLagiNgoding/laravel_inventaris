    ...
    <div class="py-12 h-full overflow-hidden">
        <div class="flex flex-col gap-4 max-w-7xl mx-auto h-full sm:px-6 lg:px-8">
            <div class="flex flex-col bg-white overflow-hidden border shadow-sm sm:rounded-lg px-6 py-14">
                <form method="post" action="{{ route('barang.edit', $barang->id) }}" class="space-y-6">
                    @csrf
                    @method ('put')

                    <div>
                        <x-input-label for="barang_name" :value="__('Nama Barang')" />
                        <x-text-input id="barang_name" name="name" type="text" class="mt-1 block max-w-md w-full" :value="$barang->name"/>
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="merk" :value="__('Merk/Type')" />
                        <x-text-input id="merk" name="merk" type="text" class="mt-1 block max-w-md w-full" :value="$barang->merk"/>
                        <x-input-error :messages="$errors->get('merk')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="category_id" :value="__('Category')" />
                        <x-select :options="$categories" :selected="$barang->category_id" class="mt-1 block max-w-md w-full" name="category_id" id="category_id" />
                        <x-input-error :messages="$errors->get('category_id')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="price" :value="__('Harga')" />
                        <x-number-input id="price" name="price" type="text" class="mt-1 block max-w-md w-full" :value="$barang->price"/>
