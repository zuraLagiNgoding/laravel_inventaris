        ...
        <div class="flex flex-col gap-4 max-w-7xl mx-auto h-full sm:px-6 lg:px-8">
            <div class="flex justify-end">                
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ url()->current() . '/export' }}" class="border border-green-600 gap-2 hover:bg-green-200/10 text-green-600 font-medium h-full px-4 py-2 rounded-lg flex items-center text-sm">Generate Riwayat Pembelian<x-bx-spreadsheet class="w-5 h-5 "/></a>
                    @endif
                    <a href="{{ url()->current() . '/save' }}" class="bg-gray-900 gap-2 text-white font-medium h-full px-4 py-2 rounded-lg flex items-center text-sm">New Pembelian<x-feathericon-plus-circle class="w-5 h-5 fill-white text-[#2D232E]"/></a>
                </div>
            </div>
                        ...
                        <tbody>
                            @foreach ($pembelians as $pembelian)    
                              <tr class="border-b h-10 border-b-slate-200 hover:bg-slate-100">
                                ...
                                <td class="px-4">{{ $pembelian->barang->merk }}</td>
                                ...
                                <td class="px-4 text-center">
                                  <div class="flex gap-2 items-center justify-center">
                                    <a href="{{ url('pembelian', $pembelian->id) }}">
                                      <x-feathericon-edit class="w-4 h-4 text-indigo-400"/>
                                    </a>
                                    <button 
                                      x-data=""
                                      x-on:click.prevent="$dispatch('open-modal', '{{ $pembelian->id }}')"
                                    >
                                      <x-feathericon-trash-2 class="w-4 h-4 text-red-400"/>
                                    </button>
                                    <x-modal name="{{ $pembelian->id }}" focusable maxWidth="md">
                                        <form method="post" action="{{ route('pembelian.destroy', $pembelian->id) }}" class="p-6">
                                            @csrf
                                            @method('delete')
