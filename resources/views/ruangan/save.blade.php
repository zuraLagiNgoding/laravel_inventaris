    ...
    <div class="py-12 h-full overflow-hidden">
        <div class="flex flex-col gap-4 max-w-7xl mx-auto h-full sm:px-6 lg:px-8">
            <div class="flex flex-col bg-white overflow-hidden border shadow-sm sm:rounded-lg px-6 py-14">
                <form method="post" action="{{ route('ruangan.save') }}" class="space-y-6">
                    @csrf

                    <div>
                        <x-input-label for="ruangan_name" :value="__('Nama Ruangan')" />
                        <x-text-input id="ruangan_name" name="name" type="text" class="mt-1 block max-w-md w-full" :value="old('name')"/>
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="deskripsi" :value="__('Deskripsi')" />
                        <x-text-input id="deskripsi" name="deskripsi" type="text" class="mt-1 block max-w-md w-full" :value="old('deskripsi')"/>
